...
func (ro *RunningOutput) Close() {
	err := ro.Output.Close()
	if err != nil {
		ro.Logger.WithField("output", ro.Name).Errorf("error closing output: %v", err)
	}
}
...
func (ro *RunningOutput) write(metrics []telegraf.Metric) error {
	...
	if err == nil {
		ro.Logger.WithField("output", ro.Name).Debugf("wrote batch of %d metrics in %s", len(metrics), elapsed)
	}
	return err
}
...
func (ro *RunningOutput) LogBufferStatus() {
	nBuffer := ro.buffer.Len()
	ro.Logger.WithField("output", ro.Name).Debugf("buffer fullness: %d / %d metrics", nBuffer, ro.MetricBufferLimit)
}
